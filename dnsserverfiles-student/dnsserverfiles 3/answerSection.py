...
class AnswerSection:
    ...
    def get_RDATA(self)-> str:
        ...
        #TODO: Student impment this method
        if self.get_TYPE() == 1:
            return str(Util.binaryToIpAddress(self.binaryString[96:128],4))
        elif self.get_TYPE() == 28:
            return str(Util.binaryToIpAddress(self.binaryString[96:224],6))
    ...
    def set_RDATA(self, _ip_address):
        #TODO: Student impment this method
        if self.get_TYPE() == 1:
            self.binaryString = self.binaryString[0:96] + Util.IpAddressToBinary(_ip_address,4) + self.binaryString[128:]
        if self.get_TYPE() == 28:
            self.binaryString = self.binaryString[0:96] + Util.IpAddressToBinary(_ip_address,6) + self.binaryString[224:]
...
class AnswerParsingManager:
    ...
    def extractAnswerObjects(_binaryString, _answer_count):
        """
        Similar to question Parsing Manager the answer parsing manager class is responsible for parsing section all answer sections
        ...
        for currentAnswer in range(0, _answer_count):
            base_of_RDLength = base + 80
            newbase = base + Util.binaryToInt(_binaryString[base_of_RDLength:base_of_RDLength+16]) * 8 + base_of_RDLength + 16
